...
const string WINDOW_TITLE = "Tai Xiu";
...
bool loadMedia()
{
	...
	// Set anywhere button:
	gButtonAnywhere.setButtonArea(&allScreen);
...
void close()
{
	...
	//Free loaded images
	gMainMenu.freeAll();
	gButtonBatDau.freeAll();
	gButtonThoat.freeAll();
	...
	for (int i = 0; i < 6; i++)
	{
		gButtonTriple[i].freeAll();
	}
	for (int i = 0; i < 14; i++)
	{
		gButtonMono[i].freeAll();
	}
	for (int i = 0; i < 6; i++)
	{
		gButtonDice[i].freeAll();
	}
	gButtonChan.freeAll();
	gButtonLe.freeAll();
	gButtonAnywhere.freeAll();
	gButtonStart.freeAll();
	gButtonClear.freeAll();
	for (int i = 0; i < xTotal; i++)
	{
		gButtonBetAmount[i].freeAll();
	}
	for (int i = 0; i < 2; i++)
	{
		gButtonMusic[i].freeAll();
		gButtonSFX[i].freeAll();
	}
	gButtonBatDau.freeAll();
	gButtonThoat.freeAll();

	Mix_FreeMusic(gMusic);
	Mix_FreeChunk(gDiceRolling);
